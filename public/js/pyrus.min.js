...
Pyrus = function( e = null, dataPYRUS = null , urlFile = null) {
    ...
    this.constructor = function() {
        ...
        if(ENTIDADES[this.entidad] === undefined) {
            console.log(`AVISO: Entidad "${this.entidad}" no encontrada`);
			return false;
        }
    ...
    this.getEspecificacion = function() {
        ...
        this.objetoSimple = {};
        for(let x in this.especificacion)
            this.objetoSimple[x] = this.especificacion[x].TIPO;
    };
    ...
    this.check = function(OBJ_elemento,STR_nombre,STR_class,OBJ_funcion = null, multiple = null) {
        ...
        if(multiple !== null) {
            nameAUX = STR_nombre.split("_");
            aux = parseInt(nameAUX[nameAUX.length - 1]);
            if(isNaN(parseInt(aux)))
                name = `${multiple}_${OBJ_elemento.NAME}_${nameAUX[nameAUX.length - 1]}[]`;
            else
                name = `${multiple}_${OBJ_elemento.NAME}[]`;
    ...
    this.inputString = function(OBJ_elemento,STR_nombre,STR_type,STR_class,OBJ_funcion = null, placeholder = "", multiple = null) {
        ...
        if(OBJ_funcion !== null) {
            for(var i in OBJ_funcion) {
                if(typeof OBJ_funcion[i] == "string") {
                    ...
                    if(STR_funcion != "") STR_funcion += " ";
                    let aux = OBJ_funcion[i].F;
                    aux = aux.replace(`/${OBJ_funcion[i].C}/`,`src-${STR_id}`);
        ...
        if(STR_type == "file") {
            let valid = OBJ_elemento.VALID;
            let invalid = OBJ_elemento.INVALID;
            let browser = OBJ_elemento.BROWSER;
            input += `<label data-invalid="${invalid}" data-valid="${valid}" class="custom-file-label mb-0 text-truncate" data-browse="${browser}" for="${STR_id}"></label>`;
            if(OBJ_elemento.SIMPLE === undefined)
                input = `<img id="src-${STR_id}" class="w-100 mx-auto d-block mb-1 img-thumbnail" src="" onError="this.src='${urlFile}'" /><div class="custom-file">${input}</div>`;
            else {
                //input = `<div class="input-group"></div>`;
                input = `<div class="input-group"><div class="custom-file">${input}</div><div class="input-group-append"><button disabled onclick="removeFile(this)" class="btn btn-danger" type="button"><i class="fas fa-times-circle"></i></button></div></div>`;
            }
        }
    ...
    this.inputText = function(OBJ_elemento,STR_nombre,STR_class,OBJ_funcion = null, placeholder = "", multiple = null) {
        ...
        if(OBJ_elemento.LABEL)
            textarea = `<label>${OBJ_elemento["NOMBRE"]}</label>${textarea}`;
        if(OBJ_elemento.FIELDSET !== undefined)
            return `<fieldset><legend>${OBJ_elemento.NOMBRE}</legend>${textarea}</fieldset>`;
		return textarea;
    }
