 ...
 * @param data - json de data que puede ser útil en los formularios, por el momento PYRUS no maneja relaciones
 */
Pyrus = function( e = null, dataPYRUS = null , urlFile = null) {
    ...
    this.columnas = function() {
        ...
        for(let x in this.especificacion) {
            ...
            Arr.push({ NAME: x.toUpperCase(), COLUMN: x, WIDTH: width, CLASS: class_ });
        }
        return Arr;
    }
        ...
        for(let i in ARR_form) {
            ...
            for(let e in row) {
                ...
                if(e == "BTN") {
                    ...
                    aux = this.inputAdecuado(this.especificacion[e], e, AUX_nombre, STR_class, OBJ_funcion, "", multiple);
                    if(this.especificacion[e].TIPO == "TP_FILE") {
                        ...
                        aux = `<img id="src-${AUX_nombre}" class="w-100 mx-auto d-block bg-dark mb-1 img-thumbnail" src="" onError="this.src='${urlFile}'" /><div class="custom-file">${aux}</div>`;
                    }
                    if(this.especificacion[e].TIPO == "TP_FECHA" || (this.especificacion[e].MULTIPLE !== undefined && this.especificacion[e].MULTIPLE))
                        ...
                        if(this.objeto.JSON[e] !== undefined) {
                            ...
                            for(let j in this.objeto.JSON[e]) {
                                if(this.especificacion[e].TIPO == "TP_FILE") {
                                    ...
                                    aux += `<div class="custom-file">${auxx}<label data-invalid="${invalid}" data-valid="${valid}" class="custom-file-label mb-0" data-browse="${browser}" for="${AUX_nombre}"></label></div>`;
                                } else
    ...
    this.inputString = function(OBJ_elemento,STR_nombre,STR_type,STR_class,OBJ_funcion = null, placeholder = "", multiple = null) {
        ...
        input = `<input value="${OBJ_elemento.DEFAULT !== undefined ? OBJ_elemento.DEFAULT : ''}" ${OBJ_elemento.READONLY === undefined ? '' : 'readonly'} ${inputData} name="${name}" id="${STR_nombre}" class="${STR_class}" type="${STR_type}" placeholder="${placeholder == "" ? OBJ_elemento["NOMBRE"] : placeholder}" />`;
    ...
    this.inputText = function(OBJ_elemento,STR_nombre,STR_class,OBJ_funcion = null, placeholder = "", multiple = null) {
        ...
        if(OBJ_elemento.FIELDSET !== undefined)
            return `<fieldset><legend>${placeholder == "" ? OBJ_elemento.NOMBRE : placeholder}</legend><textarea ${(necesario ? "required='true'" : "")} ${STR_funcion} name="${name}" id="${STR_nombre}" class="${STR_class}" placeholder="${placeholder == "" ? OBJ_elemento.NOMBRE : placeholder}"></textarea></fieldset>`;
		return `<textarea ${(necesario ? "required='true'" : "")} ${STR_funcion} name="${name}" id="${STR_nombre}" class="${STR_class}" placeholder="${placeholder == "" ? OBJ_elemento.NOMBRE : placeholder}"></textarea>`;
    }
    ...
    this.select = function(OBJ_elemento,STR_nombre,STR_class,OBJ_funcion,OBJ_datos = null,placeholder = null, multiple = null) {
        ...
        let return_STR = `<select ${(OBJ_elemento["MULTIPLE"] ? "multiple='true'" : "data-allow-clear='true'")} ${(OBJ_elemento["DISABLED"] ? "disabled='true'" : "")} ${(OBJ_elemento["NECESARIO"] ? "required='true'" : "")} ${STR_funcion} name="${name}" id="${STR_nombre}" class="${STR_class} select__2" data-tags="true" data-placeholder="${OBJ_elemento["NOMBRE"]}">`;
        if(!OBJ_elemento.MULTIPLE)
            return_STR += `<option></option>`;
            //return_STR += `<option value="" hidden selected>${OBJ_elemento["NOMBRE"]}</option>`;
        if(OBJ_datos === null) {
            if(dataPYRUS !== null) {
                if(dataPYRUS[STR_nombre] !== undefined) {
                    for(let i in dataPYRUS[STR_nombre].DATA) {
                        option = "";
                        if(dataPYRUS[STR_nombre].TIPO == "JSON") {
                            aux = JSON.parse(dataPYRUS[STR_nombre].DATA[i]);
                            for(let x in aux) {
                                if(option != "") option += " / ";
                                option += aux[x];
                            }
                        } else
                            option = dataPYRUS[STR_nombre].DATA[i]
