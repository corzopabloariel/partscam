...
<script>
    ...
    window.pyrus = new Pyrus("productoss", null, src);
    ...
    function add(t, id = 0, data = null) {
        ...
        if(id != 0)
            action = `{{ url('/adm/familias/categorias/producto/update/${id}') }}`;
        else
            action = `{{ url('/adm/familias/categorias/producto/store') }}`;
        if(data !== null) {
            console.log(data);
    ...
    erase = function(t, id) {
        ...
        alertify.confirm("ATENCIÓN","¿Eliminar registro?",
            function() {
                ...
                let promise = new Promise(function (resolve, reject) {
                    let url = `{{ url('/adm/familias/categorias/producto/delete/${id}') }}`;
                    var xmlHttp = new XMLHttpRequest();
    ...
    edit = function(t, id, modal = null) {
        $(t).attr("disabled",true);
        let promise = new Promise(function (resolve, reject) {
            let url = `{{ url('/adm/familias/categorias/producto/edit/${id}') }}`;
            var xmlHttp = new XMLHttpRequest();
            xmlHttp.responseType = 'json';
            xmlHttp.open( "GET", url, true );
            xmlHttp.onload = function() {
            ...
            promise
                .then(function(data) {
                    ...
                    if(modal === null)
                        ...
                        action = `{{ url('/adm/familias/categorias/producto/updateModal/${id}') }}`;
                        html = "";
